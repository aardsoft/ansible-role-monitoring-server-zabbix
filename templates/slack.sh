...
{% if zabbix_server.slack.webhook_passdb is defined %}
url='{{lookup(passdb, zabbix_server.slack.webhook_passdb)}}'
{% endif %}
username='Zabbix'
...
curl --insecure -m 5 --data-urlencode "${payload}" $url -A 'zabbix-slack-alertscript / https://github.com/ericoc/zabbix-slack-alertscript'
