...
if test -z "${ipmiDriverType}"; then
    echo "Missing IPMI Driver Type as the 4th parameter, see: man ipmi-sensors, look for IPMIDRIVER" >&2
    exit 1
fi
...
${IPMI_SENSORS_CMD} --driver-type=${ipmiDriverType} \
                    --hostname="${ipmiHost}" \
                    --username="${ipmiUser}" \
